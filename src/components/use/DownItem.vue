<template>
  <el-card class="box-card">
    ...
    >
      ...
      <el-table-column prop="game_password" label="游戏密码" width="180">
      </el-table-column>
      <el-table-column prop="unit_price" label="单日租金" width="80">
      </el-table-column>
      <el-table-column prop="deposit" label="押金" width="80">
      </el-table-column>
      <el-table-column prop="game_type_name" label="游戏类别" width="180" :filters="gameTypeFilters" :filter-method="filterHandler">
      </el-table-column>
...
export default {
  created() {
    ...
    this.getItemList(window.sessionStorage.getItem("uid"));
  },
  data() {
    return {
      gameList: [],
  ...
  methods: {
    ...
    async getItemList(uid) {
      const { data: res } = await this.$http.post("/gameInfo/getMyUpGameList", {
        uid: uid,
      });
